...
function CreateTrip(){
    ...
    const handleCreateTrip = async () => {
        ...
        if (!tripmate.includes(user)) {
            setErrorMsg("Current user must be in tripmate")
            return;
        }
        ...
        // let {data:{createTrip:{success, errorMessage}}} = await testMutation();
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Autocomplete
                            multiple
                            id="tags-filled"
                            options={data.AllUsers.map(({name}) => name)}
                            defaultValue={tripmate}
                            freeSolo
                            ...
                            value.map((option, index) => (
                                <Chip
                                    ...
                                    // onClick={}
                                />
                                // <AvatarChip key={option} name={option}  {...getTagProps({ index })} handleDelete={()=>{
                                //     setTripmate(tripmate.filter(tripmate=>tripmate!==option))
                                // }} />
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <FormGroup>
                            <FormControlLabel control={<Switch defaultChecked={false} />} label="Share to Public" />
                        </FormGroup>
                    </Box>
