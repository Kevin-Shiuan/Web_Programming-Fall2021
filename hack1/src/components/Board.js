...
const Board = ({ boardSize, mineNum, backToHome }) => {
    ...
    const freshBoard = () => {
        ...
        // let {temp_board,temp_Minelocation} = createBoard(boardSize,mineNum);
        let temp=Object.assign({}, createBoard(boardSize,mineNum));
        // console.log(temp);
        setBoard(temp.board);
        setMineLocations(temp.mineLocations);
        console.log("setBoard runned");
    }
        ...
        <div className = 'boardPage' >
            <div className = 'boardWrapper' >
                ...
                <div className = "boardContainer">
                    <Dashboard />
                    {/* {board.map( (r,indexX)=><div id={"row"+indexX} style = {{display: 'flex'}}>{
                        r.map( (c,indexY)=> <Cell rowIdx={indexX} colIdx={indexY}></Cell>)
                    }</div> )} */}
                    {console.log(board[0])}

                    {board.map( (r,indexX)=><div key={indexX} id={"row"+indexX} style = {{display: 'flex'}}>{
                        r.map( (c,indexY)=><Cell key={indexX+"-"+indexY} rowIdx={indexX} colIdx={indexY} detail={c} /> )
                    }</div> )} 
